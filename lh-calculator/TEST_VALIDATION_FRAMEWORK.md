...
function calculateTestPressureHot(inputs) {
  ...
  return (
    Math.ceil(
      ((1.25 * designPressure * allowableStress) / calculatedStress) * 100,
    ) / 100
  );
...
_Test Framework Version: 1.0_
_Based on: Excel ТЕСТ НОМЕР 7777_
_Status: Ready for implementation once blockers resolved_
