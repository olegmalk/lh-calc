...
export class FieldValidator {
  ...
  private buildJoiSchema(): Joi.ObjectSchema {
    ...
    schemaObj.tech_E27_weightType = Joi.string().pattern(/^[ЕК][-0-9А-Я*]*$/).required();
    schemaObj.tech_H27_quantityType = Joi.string().pattern(/^\d+\/\d+$/).required();
    ...
    schemaObj.tech_V27_thicknessType = Joi.number().min(0).optional();

    // Supply fields (снабжение) - Material specifications
    schemaObj.sup_D9_materialSpec = Joi.string()
    ...
    schemaObj.sup_K13_costQuantityNormTotal = Joi.number().min(1).max(10000).required();
    ...
    // Material codes
    schemaObj.sup_F2_parameter = Joi.string()
    ...
    schemaObj.sup_P21_priceQuantityMaterial = Joi.string()
    ...
    optionalStringFields.forEach(field => {
      schemaObj[field] = Joi.string().allow('').optional();
    });
    ...
    optionalNumberFields.forEach(field => {
      schemaObj[field] = Joi.number().min(0).optional();
    });
